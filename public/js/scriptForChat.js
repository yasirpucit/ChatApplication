$(function() {
  ...
  socket.on('onlineStack', function(stack) {
    ...
    $('#list').append(
      $('<li>').append(
        $('<button id="ubtn" class="btn btn-danger btn-block btn-lg"></button>')
          .text('Group')
          .css({ 'font-size': '18px' })
      )
    );
    ...
    for (var user in stack) {
      ...
      if (user == username) {
        var txt1 = $('<button class="boxF disabled"> </button>')
          .text(user)
          .css({ 'font-size': '18px' });
      } else {
        var txt1 = $('<button id="ubtn" class="btn btn-success  btn-md">')
          .text(user)
          .css({ 'font-size': '18px' });
      }
      //setting txt2. shows online status.
      if (stack[user] == 'Online') {
        var txt2 = $('<span class="badge"></span>')
          .text('*' + stack[user])
          .css({ float: 'right', color: '#009933', 'font-size': '18px' });
        totalOnline++;
      } else {
        var txt2 = $('<span class="badge"></span>')
          .text(stack[user])
          .css({ float: 'right', color: '#a6a6a6', 'font-size': '18px' });
      }
      //listing all users.
    ...
    if (data.room == roomId) {
      ...
      if (data.result.length != 0) {
        ...
        for (var i = 0; i < data.result.length; i++) {
          ...
          var chatDate = moment(data.result[i].createdOn).format(
            'MMMM Do YYYY, hh:mm:ss a'
          );
          var txt1 = $('<span></span>')
            .text(data.result[i].msgFrom + ' : ')
            .css({ color: '#006080' });
          var txt2 = $('<span></span>')
            .text(chatDate)
            .css({ float: 'right', color: '#a6a6a6', 'font-size': '16px' });
          var txt3 = $('<p></p>').append(txt1, txt2);
          var txt4 = $('<p></p>')
            .text(data.result[i].msg)
            .css({ color: '#000000' });
